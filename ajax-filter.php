...
$imploded_cat_id = implode(',', array_values($tt1));		
				...
				//$sqlcat = "select * from category where parent =".$catid;
				//$rescat = mysqli_query($dbLink,$sqlcat)or die("Error : ".mysqli_error());
				//while($datacat=$rescat->fetch_assoc()){	
					
         			 $query = "select * from product where Status = '1' and CategoryID IN 
					 (select CategoryID from category where parent in( ".$imploded_cat_id." ) or parent in (".$catid.") )";
					 //(select CategoryID from category where parent = ".$catid." )";
...
// $product_list;
 $prodata[] = $product_list;
					 ...
					 $query1 = "select * from product where Status = '1' and CategoryID IN (select CategoryID from category where parent = ".$catid." )";
