<?php include "db.php";
	...
	{
		//$business = AssureSec($_POST['business']);
    	...
    	if($totrec == "0")
    	{
        $sql = "insert into paypal(business,return_url,cancel_url,status,mode) values('','$return_url','$cancel_url',$status,'$mode')";
	 mysqli_query($dbLink,$sql) or die('could not insert Payment Method');
			$msg="Settings Updated Successfully";
    	}
    	else
    	{
        	 $sql = "update paypal set business='',return_url='$return_url',cancel_url='$cancel_url',status=$status,mode='$mode'";
    ...
    $sql = "Select * from paypal";
    $r = mysqli_query($dbLink,$sql) or die("can not select paypal");
    $row = $r->fetch_assoc();
    //$business = stripslashes($row['business']);
   	$return_url= stripslashes($row['return_url']);
	$cancel_url= stripslashes($row['cancel_url']);
...
<script language="javascript">
    function validate(form)
    {
        ...
        if (form.return_url.value == "")
        {
            alert("Please Enter Return URL")
            form.return_url.focus();
            return false;
        }
        if (form.cancel_url.value == "")
        {
            alert("Please Enter Cancel URL")
            form.cancel_url.focus();
            return false;
        }
...
<div id="content">
  <div class="inner">
    ...
    <div class="onecolumn" >
      <div class="header"> <span ><span class="ico  gray spreadsheet"></span> Paypal</span> </div>
      <!-- End header -->
      <div class=" clear"></div>
      <div class="content" >
          ...
          <form method="post" name="form1" id="form1" onSubmit="return validate(this);">
            <div  class="grid3">
             <!--<div class="section ">
                ...
                <div>
                  <input type="text" class="large" name="business"  id="business" value="<?=$business?>">
                </div>
              </div>-->
              <div class="section ">
                <label> Return Url<small>Text</small></label>
                <div>
                  <input type="text" class="large" name="return_url" id="return_url" value="<?=$return_url?>">
                </div>
              </div>
              <div class="section ">
                <label> Cancel Url<small>Text</small></label>
                <div>
                  <input type="text" class="large" name="cancel_url" id="cancel_url" value="<?=$cancel_url?>">
                </div>
              </div>
              <div class="section ">
