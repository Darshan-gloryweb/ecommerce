<?php include "db.php";
	...
	if(isset($_POST['Submit']))
	{
		...
		$sfromemail = $_POST['fromemail'];
		$sbcemail = $_POST['bcemail'];
		$sccemail = $_POST['ccemail'];
		$sendmail = $_POST['issend'];
		if($sendmail=='')
	    {
		   $sendmail=0;
	    }
    	...
    	if($totrec == "0")
    	{
        	$sql = "insert into websetting(sendmail,adminemail,fromemail,bcemail,ccemail) values($sendmail,'$sadminemail','$sfromemail','$sbcemail','$sccemail')";
	    	mysqli_query($dbLink,$sql) or die('could not insert..');
			$msg="Settings Updated Successfully";
    	}
    	else
    	{
        	$sql = "update websetting set sendmail='$sendmail',adminemail='$sadminemail',fromemail='$sfromemail',bcemail='$sbcemail',ccemail='$sccemail'";
    ...
    $r = mysqli_query($dbLink,$sql) or die("can not select websetting");
    $row = $r->fetch_assoc();
    $ssendmail = stripslashes($row['sendmail']);
   	$sadminemail = stripslashes($row['adminemail']);
    $sfromemail = stripslashes($row['fromemail']);
   	$sbcemail = stripslashes($row['bcemail']);
    $sccemail = stripslashes($row['ccemail']);	
            ...
            <div id="content">
				...
				<div style="margin-bottom:10px;"><a href="webset.php" class="uibutton icon confirm answer" >Back</a></div>
                            ...
                            <div  class="grid3">
                                    ...
                                    <div class="section ">
                                    <label> Send Mail<small>Mail</small></label>   
                                    <div> 
                                     <div>    
                                        <input type="checkbox" name="issend" id="issend" value="1" class="ck" <?php  if($ssendmail=='1'){ echo "checked='checked'"; } ?> />
                                        <label for="issend">Send Mail</label>
                                      </div>
                                    </div>
                                    </div>
                                    
                                     <div class="section ">
                                    <label> Admin Mail<small>Text</small></label>   
                                    <div> 
                                    <input type="text" class="large" name="adminemail" id="adminemail" value="<?=$sadminemail?>">
                                    </div>
                                    </div>
                                    <div class="section ">
                                    <label> Admin Mail From<small>Text</small></label>   
                                    <div> 
                                    <input type="text" class="large" name="fromemail" id="fromemail" value="<?=$sfromemail?>">
                                    </div>
                                    </div>
                                    <div class="section ">
                                    <label> Admin Mail CC<small>Text</small></label>   
                                    <div> 
                                    <input type="text" class="large" name="bcemail" id="bcemail" value="<?=$sbcemail?>">
                                    </div>
                                    </div>
                                    <div class="section ">
                                    <label> Admin Mail BCC<small>Text</small></label>   
                                    <div> 
                                    <input type="text" class="large" name="ccemail" id="ccemail" value="<?=$sccemail?>">
                                    </div>
                                    </div>
